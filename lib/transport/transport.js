...
var pdu          = require("../pdu/Modbus.js");
var buffer       = require("../buffer");
var EventEmitter = require("events").EventEmitter;
var Protocols = pdu.Protocols;
...
BaseTransport.prototype.send = function (fcode, extra, next) {
	...
	if (this.mutex) {
		...
		this.mutex.lock(function (unlock) {
			...
			transport.retrySend(transport.wrap(Protocols[fcode].Request.build.apply(Protocols[fcode].Request, data), extra, done), transport.retries, transport.retry, done);
		})
	} else {
		this.retrySend(this.wrap(Protocols[fcode].Request.build.apply(Protocols[fcode].Request, data), extra, next), this.retries, this.retry, next);
	}
};
				...
				if (typeof req.request.code == "number") {
					...
					transport.emit("request", req.request.code, req, function (err) {
						...
						if (err instanceof Error) {
							...
							return transport.write(transport.wrap(pdu.Exception.build(req.request.code, err), req));
						}
					...
					transport.emit("request", event_name, req, function (err) {
						...
						if (err instanceof Error) {
							return transport.write(transport.wrap(Protocols[req.request.code].Exception.build(err.code || +err.message), req));
						} else if (typeof err == "string" && err.length > 0) {
							return transport.write(transport.wrap(Protocols[req.request.code].Exception.build(err), req));
						}

						transport.write(transport.wrap(Protocols[req.request.code].Response.build.apply(Protocols[req.request.code].Response, data), req));
					});
				}
			} catch (err) {
				transport.emit("error", err);
